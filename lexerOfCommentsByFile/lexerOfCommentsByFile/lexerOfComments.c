...
int changeStage(int statusTable[5][3], int stage, int symbol) {
    int posSymbol = symbol % 42 == 0 ? 0 : symbol % 47 == 0 ? 1 : 2;
...
char* workWithStage(int statusTable[5][3], const char* fileNameWithStrings, Error* errorCheck) {
    ...
    while (fscanf(file, "%c", &symbol) == 1) {
        ...
        stage = changeStage(statusTable, stage, symbol);
    }
    fclose(file);
...
char* lexerOfComments(const char* fileNameTable, const char* fileNameWithStrings, Error* errorCheck) {
    ...
    for (int i = 1; i < 5; ++i) {
        if (fscanf(file, "%s", expression) == 1) {
            ...
            statusTable[i][2] = expression[2] - 48;
        }
    }
    fclose(file);
